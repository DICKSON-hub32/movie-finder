...
const BlogPost = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://backend.moviefinder-teckish.com/api/blogs/${slugOrId}`
        );
        const foundBlog = response.data;

        if (foundBlog) {
          setBlog(foundBlog);
          setResolvedSlug(foundBlog.slug || "");
          axios.post(
            `https://backend.moviefinder-teckish.com/api/blogs/view/${slugOrId}`
          );
        } else {
  ...
  const derivedSlug = blog.slug || slugify(blog.title);
  if (derivedSlug && derivedSlug !== slugOrId) {
    return <Navigate to={`/blog/${derivedSlug}`} replace />;
  }
  ...
  return (
    <>
      <Helmet>
        ...
        <meta
          name="description"
          content={`${blog.summary} Read the full article for more insights on ${blog.title}.`}
        />
        <meta
          name="keywords"
          content={`${blog.title.split(" ")[0]}, movie blog, film insights, ${blog.title}`}
        />
        <link
          rel="canonical"
          href={`https://moviefinder-teckish.com/blog/${resolvedSlug || derivedSlug || slugOrId}`}
        />
        <meta
          property="og:title"
          content={`${blog.title} | Movie News & Insights`}
        />
        <meta
          property="og:description"
          content={`${blog.summary} Read the full article for more insights.`}
        />
        <meta
          property="og:url"
          content={`https://moviefinder-teckish.com/blog/${resolvedSlug || derivedSlug || slugOrId}`}
        />
        <meta property="og:type" content="article" />
        <meta
          property="og:image"
          content={
            blog.imageUrl || "https://moviefinder-teckish.com/default-image.jpg"
          }
        />
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            image:
              blog.imageUrl ||
              "https://moviefinder-teckish.com/default-image.jpg",
            author: {
              "@type": "Organization",
            ...
            mainEntityOfPage: {
              "@type": "WebPage",
              "@id": `https://moviefinder-teckish.com/blog/${resolvedSlug || derivedSlug || slugOrId}`
            }
          })}
        </script>
      ...
      <div className="min-h-screen flex flex-col">
        <main className="flex-grow container mx-auto p-6">
          <div className="flex justify-between items-center mb-8">
            <h1
              className={`text-4xl font-bold text-center flex-1 ${theme === "dark" ? "text-white" : "text-black"}`}
            >
              {blog.title}
            </h1>
            {/* <button
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="lg:w-2/3">
              ...
              <p
                className={`text-lg mb-6 leading-relaxed ${theme === "dark" ? "text-gray-300" : "text-gray-800"}`}
              >
                {blog.summary}
              </p>
              <div
              ...
              >
                ...
                <p
                  className={`mb-6 leading-loose ${theme === "dark" ? "text-gray-200" : "text-gray-800"}`}
                >
                  {blog.advice}
                </p>
              </div>
                    ...
                    .map((b) => (
                      ...
                      >
                        ...
                        <p className="text-xs mt-1 text-gray-700">
                          {b.summary.split(" ").slice(0, 6).join(" ") +
                            (b.summary.split(" ").length > 6 ? "..." : "")}
                        </p>
                      </article>
                    ))}
                </div>
